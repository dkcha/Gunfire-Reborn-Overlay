              ...
              <div key={hero.id} className="bg-gray-700 p-4 rounded">
                ...
                <div className="flex gap-2 mb-2">
                  <p className="text-xs text-gray-600">
                    Skills: {hero.skills.map((s) => s.name).join(", ")}
                  </p>
                </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-8">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              {scrolls
                .filter((s) => s.rarity === "normal")
                .slice(0, 3)
                .map((scroll) => (
                  <div
                    key={scroll.id}
                    className="bg-gray-700 p-3 rounded border-l-4 border-cyan-500"
                  >
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              {scrolls
                .filter((s) => s.rarity === "rare")
                .slice(0, 3)
                .map((scroll) => (
                  <div
                    key={scroll.id}
                    className="bg-gray-700 p-3 rounded border-l-4 border-purple-500"
                  >
                    <div className="font-bold">{scroll.name}</div>
                    <div className="text-sm text-gray-300">{scroll.effect}</div>
                    <div className="flex gap-1 mt-1">
            ...
            <div className="space-y-2">
              {scrolls
                ...
                .map((scroll) => (
                  <div
                    key={scroll.id}
                    className="bg-gray-700 p-3 rounded border-l-4 border-orange-500"
                  >
                    <div className="font-bold">{scroll.name}</div>
                    <div className="text-sm text-gray-300">{scroll.effect}</div>
                    <div className="flex gap-1 mt-1">
        ...
        <div className="bg-gray-800 rounded-lg p-6 mt-8">
          ...
          <div className="bg-gray-700 p-4 rounded">
            ...
            <div className="text-green-400 font-bold">
              {(() => {
                const scrollA = gameData.getScrollById("scroll_advanced_depot");
                const scrollB = gameData.getScrollById(
                  "scroll_merciless_combo"
                );
                if (scrollA && scrollB) {
                  return gameData.checkSynergy(scrollA, scrollB)
                    ? "✓ Synergy Detected!"
                    : "✗ No Synergy";
                }
